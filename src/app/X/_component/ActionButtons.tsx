...
export default function ActionButtons({ white, post }: Props) {
    ...
    const heart = useMutation({
        mutationFn: () => {
            return fetch(`${process.env.NEXT_PUBLIC_BASE_URL}/api/posts/heart`, {
                method: 'post',
                credentials: 'include',
            })
        },
        onMutate() {
            ...
            queryKeys.forEach((queryKey) => {
                if (queryKey[0] === 'posts' && queryKey[1] === 'actions') {
                    const value: any | undefined = queryClient.getQueryData(queryKey);
                    if (value) {
                        const shallow = JSON.parse(JSON.stringify(value));
                        shallow['_count'].Hearts += 1;
                        queryClient.setQueryData(queryKey, shallow);
                        setLiked(true);
                    }
                    // console.log(queryKey[0], value);
    ...
    const repost = useMutation({
        mutationFn: () => {
            return fetch(`${process.env.NEXT_PUBLIC_BASE_URL}/api/posts/reposts`, {
                method: 'post',
                credentials: 'include',
            });
        },
